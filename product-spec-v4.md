...
Default path depends on deployment profile, but the core rule is that **native ingest and browser delivery are decoupled**. The current implementation still registers a MediaMTX passthrough relay path first (`cameras/<id>/passthrough`) so browsers and operators have a stable authenticated playback surface, but workers that must actually process frames ingest the camera RTSP URL directly instead of re-reading the relay. For `central-gpu`, the worker reads the camera directly and may then publish annotated, privacy-filtered frames to `cameras/<id>/annotated`. For `jetson-nano`, MediaMTX may expose the original camera stream as a native passthrough option **only when privacy is off**; when privacy is on, the worker must disable raw passthrough for operators and publish only privacy-filtered preview renditions instead. Passthrough/native worker modes may still read from the MediaMTX relay when that matches the desired delivery behavior. Clients pull via WebRTC (preferred), LL-HLS (iOS/fallback), or MJPEG (forensic only, and never as a privacy bypass). Overlays (bbox, class, speed) may be drawn in the worker for preview streams, and a parallel JSON telemetry stream lets the React canvas re-draw precise overlays on top of the decoded video for interactive UX.
