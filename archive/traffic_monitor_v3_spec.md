# Traffic Monitor V3 — Architecture & Implementation Design

> **Status:** Supersedes `traffic_monitor_v2_spec.md`. Retains V2's dynamic-vision-agent and centralized-VMS vision but refreshes the stack to April 2026 best practices, broadens the hybrid model so edge and central are truly first-class, and adds production-grade concerns (auth, observability, privacy, low-latency streaming).
>
> **Scope target:** mid-scale commercial VMS — 5–50 sites, 25–250 cameras.
>
> **Primary consumers of this doc:** (1) human reviewers, (2) `ai-coder-prompt-v3.md` which feeds Codex / Claude Code.
...
2. **Domain-agnostic by design.** The system is *not* a car counter. It is a general-purpose real-time multi-object detection, tracking, and analytics platform. See §10 for the full capability catalogue. In short:
   ...
   - **Rules** compose freely across class × attribute × zone × time-window × speed × proximity, with actions `count | alert | record_clip | webhook | mqtt_publish`.
...
| `keycloak` (or authentik) | Master            | OIDC provider, RBAC, multi-tenant realms                                   |
...
| Auth                  | **Keycloak** (OIDC) + FastAPI `fastapi-users` or custom middleware    | Realms = tenants; RBAC via roles; API keys for edge nodes             |
...
This is the **canonical edge device for V3**. Everything must boot on it.
...
**NVENC-free annotation strategy.** Because Orin Nano lacks a hardware encoder, V3 does *not* re-encode annotated video at the edge for re-streaming. Instead the worker:

1. Decodes RTSP with NVDEC.
2. Runs YOLO + tracker on the decoded frames (GPU).
3. **Publishes only JSON telemetry events** to NATS (`evt.tracking.<camera_id>`) — typically a few KB/s per camera.
4. Re-publishes the original RTSP stream through the local MediaMTX instance (straight passthrough, no re-encode) so the browser and HQ can still pull live video.
5. The React client draws bounding boxes / labels / speeds on an HTML `<canvas>` overlaid on the decoded `<video>` element, timestamp-aligned with the JSON events.

This keeps Orin Nano inside its thermal and bandwidth envelope while preserving the live-overlay UX. An optional fallback path uses CPU H.264 (x264 `ultrafast, zerolatency`) for a single low-res preview stream when WebRTC/canvas is unavailable — but it is never the default.
...
-- continuous aggregate: events_1m (per-camera per-class counts)
-- continuous aggregate: events_1h
...
6. **Privacy** (`privacy.py`): run secondary face + plate detectors on the already-published bbox crops; composite Gaussian blur or pixelate **onto the frame that will be streamed**. Invariant: a frame never leaves the worker without privacy applied when the camera config requires it. On Orin Nano the "frame that will be streamed" is the passthrough RTSP frame — privacy still applies to the decoded copy used for the WebRTC/canvas UX; the raw forensic stream is access-controlled separately.
7. **Secondary classifier / attribute detection** (`attributes.py`): optional second-stage model keyed off the camera's `secondary_model_id`. For each tracked detection of configured interest (e.g. every `person`), the bbox crop is passed through the secondary ONNX model (classifier or multi-label head) to emit `attributes` such as `hi_vis: true/false`, `hard_hat: true/false`, `uniform_color: "orange"`, etc. Attributes are persisted on `tracking_events.attributes` and are consumable by `detection_rules` (e.g. *"person in zone_A without hi_vis → alert"*). Models pluggable via `models` table; V3 ships with an Ultralytics-fine-tuned PPE model as a reference. Zero code changes are required to swap in a new domain model — drop the ONNX file, register it in `models`, point a camera at it.
...
All endpoints auth-gated except `/healthz`, `/readyz`, `/metrics` (internal). RBAC: `viewer` read-only, `operator` can issue commands (`/query`, start/stop), `admin` full CRUD, `superadmin` cross-tenant.
...
Default path: worker pushes annotated, privacy-filtered frames to **MediaMTX** via `rtsp://mediamtx:8554/cameras/<id>` using its `whip` or RTSP-push plugin. Clients pull via WebRTC (preferred), LL-HLS (iOS/fallback), or MJPEG (forensic only). Overlays (bbox, class, speed) are drawn **in the worker on the outbound frame** for casual viewing, and a parallel JSON telemetry stream lets the React canvas re-draw precise overlays on top of the decoded video for interactive UX.
...
* **Auth:** OIDC via Keycloak; PKCE for SPA; short-lived access tokens (15 min) + refresh. Edge nodes use scoped API keys + rotating NATS credentials.
* **RBAC:** `viewer | operator | admin | superadmin`, enforced by a FastAPI dependency. Audit log on every state-changing call.
* **Privacy enforcement:** anonymization runs in the inference worker *before* frames hit MediaMTX. A per-tenant policy can force `blur_faces=true, blur_plates=true` and prevent an operator from disabling it. Retention policy on `tracking_events` configurable per tenant (default 90 days).
...
| Measurement                                   | How it works in V3                                                                        |
|-----------------------------------------------|-------------------------------------------------------------------------------------------|
...
Each rule has an **action** (`count | alert | record_clip | webhook | mqtt_publish`) and a cooldown. Webhook payloads include the snapshot URL from MinIO and the full rule context.
...
- **Full behavioral / action recognition** (e.g. "is this person falling?") — a posture / action classifier can be plugged in as a secondary model, but V3 does not include a bundled action-recognition network out of the box.
- **Video LLM scene understanding** — the LLM adapter drives configuration (class filters, rule generation), not per-frame captioning. Scene-understanding via VLM is a roadmap item for V3.x.

---

## 11. Migration Path from V1

**There is no migration path.** V1 and V3 are separate systems.
...
* **Data:** if the operator wants to preserve the V1 `traffic.db` historical counts for reference, a one-shot standalone script (outside the V3 repo, under `scripts/legacy/import-v1-sqlite.py`) can read `traffic.db` and insert rows into V3's `tracking_events` as `class_name='car'`, `speed_kph=NULL`, tagged to a synthetic site/camera named `Legacy V1`. This script is **not** part of the V3 runtime and should not appear in any production deploy.
...
The blueprint file `ai-coder-prompt-v3.md` drives step-by-step implementation. Each prompt in that file must, on completion, leave the repo in a state where:
