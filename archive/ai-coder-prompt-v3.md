...
> Create the exact folder structure from §6 of the spec. In `backend/`, initialize a `uv` project (`pyproject.toml`) with dependency groups `runtime`, `dev`, `vision`, `llm`. Runtime deps include: `fastapi`, `uvicorn[standard]`, `pydantic>=2`, `pydantic-settings`, `sqlalchemy[asyncio]`, `asyncpg`, `alembic`, `redis`, `nats-py`, `httpx`, `structlog`, `opentelemetry-api/sdk/exporter-otlp`, `python-jose[cryptography]`, `passlib[argon2]`, `cryptography`. Vision deps: `opencv-python-headless`, `onnxruntime-gpu` (with a CPU extra), `ultralytics`, `numpy<2`, `Pillow`. LLM deps: `litellm`, `instructor`, `ollama`. Dev deps: `ruff`, `mypy`, `pytest`, `pytest-asyncio`, `testcontainers[postgres,redis,nats]`, `playwright`.
>
...
>
> Implement `core/security.py`: OIDC validator for Keycloak (JWKS cached 1h), FastAPI `CurrentUser` dependency, RBAC decorator `require(role)`, edge-key middleware that validates scoped API keys for `/api/v1/edge/*`. RTSP URL encryption helpers (AES-GCM).
>
...
>
> - `profile=jetson-nano` (default on `arm64` when NVENC is absent): publish **only** JSON events to NATS. Do **not** re-encode an annotated stream. MediaMTX on the edge ingests the original RTSP and re-serves it (passthrough) to the browser, which overlays boxes on an HTML canvas using the JSON events. This is the supported path on Jetson Orin Nano Super.
...
>
> Implement `streaming/mediamtx.py` — two modes: (a) RTSP passthrough (register a pull source for the camera's RTSP URL), used by the jetson-nano profile; (b) WHIP push of annotated frames, used by central-gpu profile.
>
...
>
> **Verification:** gates; end-to-end test: seed a camera row pointing at a looping test RTSP published by MediaMTX → worker runs → `tracking_events` rows accumulate → MediaMTX exposes the (passthrough or annotated, depending on profile) stream. Run the end-to-end test **twice**: once under docker-compose on x86 with the `central-gpu` profile, and once on a Jetson Orin Nano Super with the `jetson-nano` profile (or its CI emulation via `qemu-system-aarch64` + a recorded RTSP fixture).

---

## Prompt 5 — FastAPI master node: CRUD, edge APIs, LLM adapter

> Implement routers in `api/v1/` for `sites`, `cameras`, `edge`, `query`, `history`, `export`, `incidents`, `streams`, and a WS handler at `/ws/telemetry`. All routes use the RBAC dependency from Prompt 2.
>
> `sites` and `cameras` are full CRUD with Pydantic v2 request/response models. Camera create/update validates the 8 homography points + ref distance and the `privacy` object against the tenant policy (privacy policy may force certain flags true).
>
...
>
> `POST /api/v1/streams/{camera_id}/offer` implements WebRTC signaling by proxying to MediaMTX's WHEP endpoint.
>
...
> Finalize MediaMTX configuration in `infra/mediamtx/mediamtx.yml`: auth via JWT, enable WebRTC + LL-HLS + MJPEG, CORS for the SPA origin, per-stream publish auth keyed off the camera's id.
>
...
>
> **Verification:** gates; opening the MediaMTX web UI shows the annotated stream for a running camera; a plain HTML test page consumes the WebRTC offer route successfully.
...
4. Commit (`feat: scaffold v3 monorepo`), then paste **Prompt 2**. Repeat.
5. Never skip the gates between prompts — debt compounds fast in a multi-service system like this.
6. If Codex proposes a change that conflicts with `traffic_monitor_v3_spec.md`, it must stop and ask. Reject silent divergence.
