...
This compose command is appropriate for lab and pilot edge bring-up. In
product-mode edge installs, prefer `./bin/vezor install edge`; the dev-manifest
path resolves the Jetson GPU ONNX Runtime wheel from manifest and preflight
data. In production, run the same edge responsibilities under the chosen
supervisor model so workers restart after reboot, expose runtime state, and
receive lifecycle commands from the control plane.
