...
- soak validated runtime artifacts on real Jetson deployments. Fixed-vocab `.engine` artifacts and compiled per-scene open-vocab artifacts have landed as runtime artifacts, but they still need first-site performance and reliability validation before being called fleet-safe.
