...
async function installUiAuditFixtures(page: Page) {
  await page.addInitScript(() => {
    Object.defineProperty(window, "__VEZOR_CONFIG__", {
      value: {
    ...
    const oidcUser = (authority: string) =>
      JSON.stringify({
    ...
    Object.defineProperty(window, "RTCPeerConnection", {
      value: class UnavailablePeerConnection {
        constructor() {
          throw new Error("E2E UI audit uses mocked stream transport.");
        }
      },
      configurable: true,
    });
  ...
  await page.route("**/api/v1/operations/memory-patterns**", async (route) => {
...
async function countPanelLikeSurfaces(page: Page) {
  return page.evaluate(() => {
    return Array.from(document.querySelectorAll("body *")).filter((element) => {
      ...
      return (
        rect.width >= 40 && rect.height >= 24 && (borderWidth > 0 || hasSurface)
      );
    }).length;
  });
}

test("dashboard does not render the moving 3D brand object", async ({
  page,
}) => {
...
test("operations default surface count stays below the overload budget", async ({
  page,
}) => {
  await installUiAuditFixtures(page);
