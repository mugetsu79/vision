      ...
      <div className="grid gap-3 px-5 py-5">
        {entries.length === 0 ? (
          ...
          entries.map(([className, count]) => (
            <div
              key={className}
              className="rounded-[1.35rem] border border-white/8 bg-[linear-gradient(180deg,rgba(14,24,39,0.98),rgba(8,14,23,0.98))] px-4 py-3"
            >
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#88a3cc]">
                {className}
              </p>
              <p className="mt-2 text-2xl font-semibold text-[#f3f7ff]">{count}</p>
            </div>
          ))
        )}
      </div>
