...
@pytest.mark.asyncio
async def test_query_series_materializes_aligned_buckets_for_non_aligned_window(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = HistoryService(session_factory=MagicMock())
    service._ensure_camera_access = AsyncMock()
    starts = datetime(2026, 4, 26, 14, 2, tzinfo=UTC)
    ends = starts + timedelta(minutes=10)
    aligned_populated_bucket = datetime(2026, 4, 26, 14, 5, tzinfo=UTC)
    monkeypatch.setattr(
        service,
        "_fetch_series_rows_from_events",
        AsyncMock(
